...
export function ComposeDialog({ 
  ...
  const insertImageToBody = (imageUrl: string) => {
    // For rich text, we'd insert HTML, but for plain text we add a placeholder
    const imageTag = `\n[Image: Generated by AI]\n`;
    setBody(prev => prev + imageTag);
    
    // Store the image in session for attachment handling
    // In a real implementation, you'd upload this to storage and attach it
    toast({
      title: "Image added",
      description: "The image will be embedded when you send the email",
    });
              ...
              <Popover open={showImageGenerator} onOpenChange={setShowImageGenerator}>
                <PopoverTrigger asChild>
                  <Button
                    ...
                    className={`h-9 gap-2 ${isPro ? 'border-primary/30 bg-primary/5 text-primary' : ''}`}
                    ...
                    <div className="space-y-3 text-center py-2">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-muted-foreground">
                          Upgrade to Pro to generate AI images for your emails
                        </p>
                      </div>
                    </div>
                  )}
