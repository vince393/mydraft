...
- **Email Threading (`threadId`)**: Inbox/detail group emails by `threadId`. Gmail uses its native `threadId`, Outlook uses `conversationId`. IMAP has no native thread id, so `server/imap.ts` derives a stable conversation key (`deriveImapThreadId`): the thread root = first id in the `References` header, else `In-Reply-To`, else the message's own Message-ID, else UID. All candidate ids are normalized (`normalizeMessageId`: strip `<>`, trim, lowercase) so the list path (ImapFlow envelope returns bare ids) and the detail path (mailparser/References return bracketed ids) produce the same key. The list fetch requests `headers: ["references"]`. `threadId` is persisted in the `cached_emails` table during sync, so grouping survives reloads.
